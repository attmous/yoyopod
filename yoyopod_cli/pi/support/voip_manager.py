...
from yoyopod_cli.pi.support.call_history import CallHistoryEntry
from yoyopod_cli.pi.support.voice_note_draft import VoiceNoteDraft
...
def _duration_ms(value: object) -> int:
    return _duration_seconds_from_value(value)
...
def _snapshot_failure_text(snapshot: VoIPRuntimeSnapshot) -> str:
    ...
    if message is not None and message.error.strip():
        return message.error.strip()
    lifecycle_reason = snapshot.lifecycle.reason.strip()
    if lifecycle_reason:
