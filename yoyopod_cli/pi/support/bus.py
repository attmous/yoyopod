"""CLI-owned typed event bus used by Pi validation helpers."""
...
class Bus:
    ...
    def subscribe(self, event_type: type[Any], handler: EventHandler) -> None:
        ...
        logger.trace("Subscribed CLI support bus handler for {}", event_type.__name__)
    ...
    def _dispatch(self, event: Any) -> None:
        ...
        for handler in handlers:
            try:
                ...
                if self._strict:
                    raise
                logger.exception("Error handling CLI support event {}", event.__class__.__name__)
