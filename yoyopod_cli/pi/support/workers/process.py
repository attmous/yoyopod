...
class WorkerProcessRuntime:
    ...
    def __init__(self, config: WorkerProcessConfig) -> None:
        ...
        self._outbound: Queue[WorkerEnvelope | None] = Queue(maxsize=max(1, config.send_queue_size))
    ...
    def _start_writer(self) -> None:
        ...
        thread = threading.Thread(
            ...
            name=f"yoyopod-cli-worker-{self.config.name}-stdin",
            daemon=True,
        )
        thread.start()
    ...
    def _start_reader(self, stream_name: str, stream: IO[bytes]) -> threading.Thread:
        thread = threading.Thread(
            ...
            name=f"yoyopod-cli-worker-{self.config.name}-{stream_name}",
            daemon=True,
        )
        thread.start()
        return thread
