"""Tests for the scaffold diagnostics integration."""
...
from yoyopod.integrations.diagnostics import setup, teardown
...
def test_diagnostics_setup_records_events_and_writes_snapshot(tmp_path: Path) -> None:
    ...
    app.start()
    app.states.set("screen.awake", True, {"source": "test"})
    app.tick()

    log_entries = app.log_buffer.snapshot()
    assert integration is app.integrations["diagnostics"]
    assert any(
    ...
    assert any(
        ...
        and entry["payload"]["entity"] == "screen.awake"
        for entry in log_entries
    )
    ...
    assert payload["states"]["screen.awake"]["value"] is True
    assert payload["states"]["screen.awake"]["attrs"] == {"source": "test"}
