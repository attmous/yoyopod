...
import WhitelistLayers from '../../../../assets/diagrams/whitelist-layers.svg';
...
The target is the full Talk promise: calls and voice messages that only family can place or receive, with whitelist enforcement architected end to end (the design above) and validated automatically on hardware. [The yoyocore Runtime](/builders/software/runtime/) covers how workers like this one are supervised along the way.

*Design direction — not built yet.* The target also requires deciding what the voip worker registers *against* — the SIP infrastructure decision. Nothing is committed today; the realistic options are:
...
:::note[Sources]
Condensed from [`docs/product/PRODUCT_DEFINITION.md`](https://github.com/attmous/yoyopod/blob/main/docs/product/PRODUCT_DEFINITION.md) and the as-built docs site (docsite/website/ in the repository): the VoIP switchboard worker page, the product definition page, and the roadmap honesty page.
:::
