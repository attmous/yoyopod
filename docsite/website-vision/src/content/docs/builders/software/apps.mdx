...
import PairingFlow from '../../../../assets/diagrams/pairing-flow.svg';

*The planned apps/ and packages/ layer: the yoyopod app and the contracts it shares with the device.*

:::note[The vision]
This page describes the app platform yoyopod is designed to grow into — nothing
in this layer is built yet. What's real right now is documented in the
...
**Today: nothing in this layer exists.** `apps/` and `packages/` are planned directories, not present ones. There is no parent app, no shared-contract package, and no app↔yoyocloud protocol in use. Today's device and cloud sides are documented in the as-built engineering docs (`docsite/website/` in the repository).
...
*The key design decisions still open.*
