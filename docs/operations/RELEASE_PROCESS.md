...
- package version source: `yoyopod_cli/_version.py`
- release tag format: `vMAJOR.MINOR.PATCH`
- release artifacts: Python CLI package distributions, full repo bundles, and an ARM64 slot tarball
...
3. runs the configured CI jobs for Rust runtime code and Python CLI/deploy tooling
4. runs `uv run yoyopod release build --check-tag <tag>`
5. builds a Linux ARM64 self-contained slot artifact via `deploy/docker/slot-builder.Dockerfile`
...
- The Python package artifacts are for CLI/deploy tooling, not a Python app runtime.
