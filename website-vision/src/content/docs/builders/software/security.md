...
- **Whitelist enforcement — is the two-layer device-side gate (as proposed, matching the VoIP Engine) sufficient, or do we require a SIP path that also allows server-side rejection?** Requiring the third layer constrains the self-hosted-vs-managed SIP decision.
